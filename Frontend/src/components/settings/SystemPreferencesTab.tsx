// src/components/settings/SystemPreferencesTab.tsx
...
import { currencyList, timeZoneList } from "@/utils/global";
...
interface SystemSettingsData {
  ...
  allow_higher_branch_access: number | null;
  higher_branch_allowed_days: number | null;
...
export default function SystemPreferencesTab({
  ...
  const [formData, setFormData] = useState<SystemSettingsData>(initialData);
  ...
  // const handleSave = () => {
  //   const payload: Partial<SystemSettingsData> = {
  //     ...formData,
  //     higher_branch_allowed_days:
  //       formData.allow_higher_branch_access === 1
  //         ? formData.higher_branch_allowed_days
  //         : null,
  //   };
  ...
  setFormData({
    ...initialData,
    allow_higher_branch_access:
      initialData.allow_higher_branch_access === 1 ? 1 : null,
    higher_branch_allowed_days:
      initialData.allow_higher_branch_access === 1
        ? initialData.higher_branch_allowed_days
        : null,
  });
}, [initialData]);
  ...
  payload.append("email_notification_status", String(formData.email_notification_status));


  if (formData.allow_higher_branch_access !== null) {
  payload.append("allow_higher_branch_access", String(formData.allow_higher_branch_access));
  }
  ...
  return (
    <TabsContent value="system" className="mt-6 space-y-6">
      {/* === NEW: Profile Update Card (at the very top) === */}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure global settings for your gym
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Timezone */}
          {/* <div className="space-y-2">
            ...
            <Select
              value={formData.timezone_id.toString()}
              onValueChange={(val) => handleChange("timezone_id", Number(val))}
            >
              <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Input
              value={
                currencyList.find(c => c.id === formData.currency_id)?.text || ""
              }
              disabled
            />
          </div>
          ...
          {/*<div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                Send email alerts for memberships, payments, and events
              </p>
            </div>
            <Switch
              checked={formData.email_notification_status === 1}
              onCheckedChange={(checked) =>
                handleChange("email_notification_status", checked ? 1 : 0)
              }
            />
          </div> */}

          {/* Allow Higher Branch Access + Days */}
          <div className="space-y-6 p-4 bg-muted/30 rounded-lg border">
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) => {
                  setFormData((prev) => ({
                    ...prev,
                    allow_higher_branch_access: checked ? 1 : null,
                    higher_branch_allowed_days: checked
                      ? prev.higher_branch_allowed_days || 1
                      : null,
                  }));
                }}
              />
            ...
            {/* Only show days input if higher branch access is enabled */}
            {formData.allow_higher_branch_access === 1 && (
              <div className="space-y-2 mt-4">
                <Label htmlFor="higher_branch_days">
          ...
          <div className="pt-6 border-t">
            <Button
              permission={[PERMISSIONS.SYSTEM_SETTINGS_CREATE, PERMISSIONS.SYSTEM_SETTINGS_EDIT]}
              onClick={handleSave}
              disabled={isLoading}
              size="lg"
              className="w-full sm:w-auto"
